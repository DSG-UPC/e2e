...
//const web3_2 = require('./web3_2.js');
const fs = require('fs')
...
function writeJSON() {
    ...
    // }, 0);
    if (endedTxs == nTx) {
        const jsonAcc = JSON.stringify(jsonAccounts, null, "\t");
        //console.log(json)
        fs.writeFile(`./accounts.json`, jsonAcc, err => {
            if (err) {
        ...
        tok.methods.balanceOf(jsonAccounts[0].address).call({ from: accounts[0] }).then((res, err) => {
            console.log("account1: "+res)
        })
        tok.methods.balanceOf(jsonAccounts[1].address).call({ from: accounts[0] }).then((res, err) => {
            console.log("account2: "+res)
        })
        tok.methods.balanceOf(jsonAccounts[4999].address).call({ from: accounts[0] }).then((res, err) => {
            console.log("account4999: "+res)
        })
    }
}
...
function sendTx(nonce) {
    for (let id = 0; id < nTx; id++) {
        // if(id%2==0)web3tmp = web3
        // else web3tmp=web3_2
        if(accountsIt>=jsonAccounts.length) accountsIt = 0
        destAccountsIt = accountsIt+1
        if(destAccountsIt>=jsonAccounts.length) destAccountsIt = 0
        nonce =jsonAccounts[accountsIt].nonce
        jsonAccounts[accountsIt].nonce = nonce+1
        ...
        const tx = {
            from: jsonAccounts[accountsIt].address,
            to: tokAt,
            gas: 200000,
            data: tok.methods.transfer(jsonAccounts[destAccountsIt].address, 2).encodeABI()
        }
        const signPromise = web3.eth.accounts.signTransaction(tx, jsonAccounts[accountsIt].privateKey);
        signPromise.then((signedTx) => {
            ...
            //console.log(err)
            console.log("Error on tx: " + id)
        });

        // const signedTx = await web3.eth.accounts.signTransaction(tx, jsonAccounts[accountsIt].privateKey);
        // web3.eth.sendSignedTransaction(signedTx.rawTransaction)
        //     //.once('sending', function(payload){ results[id]['sendingTime']=Date.now() })
        //     .once('sent', function (payload) {
        //         results[id]['sentTime'] = Date.now()
